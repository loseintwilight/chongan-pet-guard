...
export default {
  ...
  methods: {
    ...
    getFileDownloadUrl(fileName) {
      if (isExternal(fileName) || fileName.startsWith(this.baseUrl)) {
        return fileName;
      }
    ...
    handleUploadSuccess(res, file) {
      if (res.code === 200) {
        const downloadUrl = this.getFileDownloadUrl(res.fileName);
        this.uploadList.push({name: res.fileName, url: downloadUrl})
