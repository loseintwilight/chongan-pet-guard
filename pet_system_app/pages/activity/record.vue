...
<script>
import { listActivity, getUserActivityList } from '@/api/pet/activity';
...
export default {
	...
	methods: {
		...
		getList() {
			...
			getUserActivityList(this.queryParams)
				.then((response) => {
		...
		navigateToDetail(activity) {
			uni.navigateTo({
				url: '/pages/activity/recordDetail?id=' + activity.activityId
			});
		},
