	...
	<view class="detail-container">
		...
		<view class="action-bar">
			<view class="action-btn contact-btn" @click="contactOwner">
				<text class="btn-text">联系主人</text>
			</view>
			<view class="action-btn share-btn" @click="sharePet">
...
export default {
	...
	methods: {
		...
		async loadPetDetail() {
			...
			try {
				const res = await getLostPet(this.lostId);
				console.log('接口完整响应:', res);
				console.log('响应code:', res.code);
				console.log('响应data:', res.data);
				console.log('响应rows:', res.rows);
				console.log('响应msg:', res.msg);

				if (res.code === 200) {
					// 根据PC端接口和领养详情页的代码，数据应该在data字段
					const data = res.data;
					console.log('获取到的宠物数据:', data);

					if (data) {
						// 使用对象赋值，确保所有字段都被覆盖
						Object.keys(this.petDetail).forEach((key) => {
							if (data[key] !== undefined) {
								this.$set(this.petDetail, key, data[key]);
							}
						});
		...
		// 分享
		sharePet() {
			const content = `【寻宠启事】${this.petDetail.petName}在${this.petDetail.lostLocation}走失，电话：${this.petDetail.ownerPhone || '暂无'}`;
